...
function buildInsights(summary, gst, clientGrowth) {
    const insights = [];
...
exports.reportRouter.get("/dashboard", async (req, res, next) => {
    try {
        const userId = req.user.id;
        const [summaryRows] = await db_1.pool.execute(`SELECT
       ...
       FROM invoices WHERE user_id = :userId`, { userId });
        const summary = summaryRows[0] ?? {};
        const [monthlyRevenue] = await db_1.pool.execute(`SELECT DATE_FORMAT(issue_date, '%Y-%m') month,
       ...
       ORDER BY month DESC
       LIMIT 12`, { userId });
        const [paymentTrends] = await db_1.pool.execute(`SELECT DATE_FORMAT(COALESCE(p.created_at, i.paid_at), '%Y-%m') month,
       ...
       ORDER BY month DESC
       LIMIT 12`, { userId });
        const [clientGrowth] = await db_1.pool.execute(`SELECT DATE_FORMAT(created_at, '%Y-%m') month, COUNT(*) clients
       FROM clients
       WHERE user_id = :userId
       GROUP BY month
       ORDER BY month DESC
       LIMIT 12`, { userId });
        const [topClients] = await db_1.pool.execute(`SELECT c.id, c.name, c.email,
       ...
       LIMIT 5`, { userId });
        const [gstRows] = await db_1.pool.execute(`SELECT
        COALESCE(SUM(tax_total), 0) gst_total,
        COALESCE(SUM(subtotal), 0) taxable_value,
        COALESCE(AVG(NULLIF(tax_total, 0)), 0) average_tax
       FROM invoices WHERE user_id = :userId`, { userId });
        const gstSummary = gstRows[0] ?? {};
        const [recentActivity] = await db_1.pool.execute(`SELECT 'invoice' type, invoice_number title,
       ...
       LIMIT 10`, { userId });
        res.json({
            ...
            clientGrowth: clientGrowth.reverse(),
            topClients,
            gstSummary,
            recentActivity,
            insights: buildInsights(summary, gstSummary, clientGrowth),
        });
    }
    catch (error) {
        next(error);
...
exports.reportRouter.get("/exports", async (req, res, next) => {
    try {
        const userId = req.user.id;
        const [revenue] = await db_1.pool.execute(`SELECT DATE_FORMAT(issue_date, '%Y-%m') month, COUNT(*) invoices, COALESCE(SUM(total),0) total
       FROM invoices WHERE user_id = :userId GROUP BY month ORDER BY month DESC`, { userId });
        const [gst] = await db_1.pool.execute(`SELECT invoice_number, issue_date, customer_name, customer_tax_id, subtotal taxable_value, tax_total, total, currency
       FROM invoices WHERE user_id = :userId ORDER BY issue_date DESC`, { userId });
        const [clients] = await db_1.pool.execute(`SELECT c.name, c.email, COUNT(i.id) invoices, COALESCE(SUM(i.total),0) revenue
       FROM clients c LEFT JOIN invoices i ON i.client_id = c.id
       WHERE c.user_id = :userId GROUP BY c.id ORDER BY revenue DESC`, { userId });
        const [expenses] = await db_1.pool.execute(`SELECT DATE_FORMAT(expense_date, '%Y-%m') month, COUNT(*) expenses, COALESCE(SUM(total),0) total
       FROM expenses WHERE user_id = :userId GROUP BY month ORDER BY month DESC`, { userId });
        const [profit] = await db_1.pool.execute(`SELECT r.month, r.revenue, COALESCE(e.expenses,0) expenses, r.revenue - COALESCE(e.expenses,0) profit
       FROM (
