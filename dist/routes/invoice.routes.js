...
async function clientIdForInvoice(userId, body, db) {
    if (body.clientId)
    ...
    if (!customerName)
        return null;
    const [existingRows] = await db.execute(`SELECT id FROM clients
     ...
     ORDER BY id DESC
     LIMIT 1`, { userId, email: customerEmail, taxId: customerTaxId, name: customerName });
    const existing = existingRows[0];
    if (existing?.id)
        return existing.id;
    const [result] = await db.execute(`INSERT INTO clients (user_id, name, email, tax_id, billing_address, notes)
    ...
    try {
        ...
        const { invoiceId, invoiceNumber, pdfStyle } = await (0, db_1.withTransaction)(async (connection) => {
         ...
         VALUES (:userId, :clientId, :invoiceNumber, :issueDate, :dueDate, :status, :currency, :businessName, :businessEmail,
          ...
          :pdfStyle, :subtotal, :taxTotal, :discountTotal, :total)`, { userId: req.user.id, ...invoiceSqlParams({ ...body, clientId }, totals, pdfStyle), invoiceNumber });
...
exports.invoiceRouter.put("/:id", async (req, res, next) => {
    try {
        ...
        const { pdfStyle, invoiceNumber } = await (0, db_1.withTransaction)(async (connection) => {
         ...
         discount_total=:discountTotal, total=:total WHERE id=:id AND user_id=:userId`, { id: req.params.id, userId: req.user.id, ...invoiceSqlParams({ ...body, clientId }, totals, pdfStyle), invoiceNumber });
