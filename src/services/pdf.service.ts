...
interface PdfTheme {
  ...
  category?: "classic" | "modern" | "gst" | "agency" | "construction" | "retail" | "minimal" | "document";
  noLogo?: boolean;
  watermark?: "waves" | "leaf" | "rose";
}
...
function themeAllowsLogo(theme: PdfTheme) {
  return !theme.noLogo;
}
...
function drawWatermark(doc: PDFKit.PDFDocument, theme: PdfTheme) {
  if (!theme.watermark) return;
  doc.save();
  doc.opacity(0.12);
...
function drawHeader(doc: PDFKit.PDFDocument, invoice: Record<string, unknown>, theme: PdfTheme, settings?: Record<string, unknown>) {
  ...
  if (theme.header === "solid") {
    ...
    if (themeAllowsLogo(theme)) drawLogo(doc, settings, 48, 38, 42);
  ...
  if (theme.header === "split") {
    ...
    if (themeAllowsLogo(theme)) drawLogo(doc, settings, 48, 38, 38);
  ...
  if (theme.header === "side") {
    ...
    if (themeAllowsLogo(theme)) drawLogo(doc, settings, 52, 38, 38);
  ...
  if (theme.header === "bar") {
    ...
    if (themeAllowsLogo(theme)) drawLogo(doc, settings, 48, 58, 38);
